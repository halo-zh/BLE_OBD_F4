...
void isotp_on_can_message(IsoTpLink *link, uint8_t *data, uint8_t len) {
    ...
    memset(message.as.data_array.ptr + len, 0, sizeof(message.as.data_array.ptr) - len);   // set unused data to 0
...
int isotp_receive(IsoTpLink *link, uint8_t *payload, const uint16_t payload_size, uint8_t *out_size) {
    uint16_t copylen;
